    ...
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_18rem] lg:items-start">
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-slate-50">
          {form.image_url ? (
            <img alt={form.name || "Product preview"} className="h-48 w-full object-cover" src={form.image_url} />
          ) : (
            <div className="flex h-48 items-center justify-center text-sm text-slate-400">No image selected</div>
          )}
          <div className="border-t border-slate-200 px-4 py-3 text-xs text-slate-500">Primary image preview</div>
        </div>
      </div>

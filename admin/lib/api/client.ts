...
export function updateAdminCategory(categoryId: number, payload: {
  name: string;
  slug?: string | null;
  parent_id?: number | null;
  position?: number;
}): Promise<Category> {
  return apiFetch<Category>(toUniformaProxy(apiEndpoints.adminCategory(categoryId)), {
    method: "PUT",
    body: JSON.stringify(payload)
  });
}
...
export function deleteAdminCategory(categoryId: number): Promise<void> {
  return apiFetch<void>(toUniformaProxy(apiEndpoints.adminCategory(categoryId)), {
    method: "DELETE"
  });
}
...
export function deleteAdminMedia(filename: string): Promise<void> {
  return apiFetch<void>(toUniformaProxy(apiEndpoints.adminMediaItem(encodeURIComponent(filename))), {
    method: "DELETE"
  });
}
...
export function getSupplierSyncOverview(): Promise<SupplierSyncOverview> {
  return apiFetch<SupplierSyncOverview>(toUniformaProxy(apiEndpoints.adminSupplierSync));
}

export function getAdminAnalytics(): Promise<AdminAnalytics> {
  return apiFetch<AdminAnalytics>(toUniformaProxy(apiEndpoints.adminAnalytics));
}

export function getAdminContent(): Promise<SiteContent> {
  return apiFetch<SiteContent>(toUniformaProxy(apiEndpoints.adminContent));
}

export function updateAdminContent(payload: SiteContent): Promise<SiteContent> {
  return apiFetch<SiteContent>(toUniformaProxy(apiEndpoints.adminContent), {
    method: "PUT",
    body: JSON.stringify(payload)
  });
...
export function getCatalogProducts(query: Record<string, string | number | string[] | undefined>): Promise<CatalogProductList> {
  const url = new URL(toUniformaProxy(apiEndpoints.publicProducts), "http://localhost");
  ...
  return apiFetch<CatalogProductList>(url.pathname + url.search);
}
...
export function getSearchSuggestions(query: string): Promise<SearchSuggestionResponse> {
  return apiFetch<SearchSuggestionResponse>(toUniformaProxy(apiEndpoints.publicSearchSuggestions), {
    query: { q: query }
  });
}
